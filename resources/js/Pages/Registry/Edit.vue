            ...
            <div class="sticky top-5 mb-5">
                <div class="card p-7 sm:w-72" :class="{ 'border border-red-500 dark:border-red-500' : errors.category_id}">
                    ...
                    <div v-for="item in categories" :key="item.id">
                        ...
                        <label :for="'c_'+item.id" class="cursor-pointer block p-2 px-4 mb-4 rounded-xl bg-gray-200/40 dark:bg-slate-600/30 peer-checked:bg-gray-300 dark:peer-checked:bg-slate-600 dark:peer-checked:text-slate-300">{{ item.name }}</label>
                    </div>
        ...
        <div class="card grid grid-cols-1 gap-4 sm:p-9 sm:mx-6 w-full max-w-screen-2xl">
            <div class="relative grid grid-cols-1 sm:grid-cols-5 gap-x-4 bg-gray-200 dark:bg-slate-600 dark:text-slate-300 p-4 rounded-xl
                ...
                :class="status_color(form.status_id)">
                    ...
                    <div class="sm:row-start-2 sm:col-start-3 sm:col-span-3 self-end sm:text-right text-sm text-slate-400"> <span class="mr-5">{{ form.term ? 'До ' + form.term : 'Бессрочный' }}</span> {{ status_name ? status_name : 'Без статуса' }}</div>
            </div>
            ...
            <div class="relative h-full text-slate-600 dark:text-slate-300 bg-slate-300/20 ckdrop-blur-sm rounded-xl p-6 mx-3" v-if="form.files.length">
                <div class="flex overflow-x-auto" >
                    <div class="relative block text-center hover:bg-slate-300/30 p-3 rounded-lg w-32 z-50" v-for="file in form.files" :key="file" >
                        <div class="absolute top-3 right-3 rounded-full bg-emerald-500 text-white w-5 h-5 flex justify-center items-center text-xs leading-none"
                        v-if="file.id"><i class="fi fi-rr-check"></i></div>
                        <div class="text-4xl"><i class="fi fi-rr-file"></i></div>
                        <div class="text-xs h-6 marquee">{{ file.name || file.file.split('--&')[1] }}</div>
                        <div class="text-xs text-slate-500/70 dark:text-slate-300/50">{{ file.size ? filesize(file.size) : 'Неизвестно' }}</div>
                        <button @click.prevent="deleteFile(file)" class="text-red-600/90 dark:text-red-400" title="Удаляет файл с сервера" v-if="file.id">Удалить</button>
                        <button @click.prevent="removeFile(file)" class="text-red-600/90 dark:text-red-400" title="Убирает файл перед загрузкой" v-else>Удалить</button>
                    </div>
                </div>
            </div>

            <loading-button :loading="form.processing" class="btn-green mx-auto w-full max-w-xs" type="submit">Обновить</loading-button>
            <button @click.prevent="destroy" class="text-red-600/90 dark:text-red-400">Удалить реестр</button>
        </div>
...
export default {
    ...
    mounted() {
        this.$nextTick(() => {
            this.form.files = this.regist.files;
        })
    },
    methods: {
        filesize(size) {
